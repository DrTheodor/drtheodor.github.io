...
import { parseColor, Random } from "../util";
...
export class Nebula extends Drawable {
  intensity: number;
  ...
  constructor({
    ...
    this.colorations = COLORS.flatMap((color) => {
      return new Array(ITERATION_PER_COLOR).fill(0).map(() => {
        ...
        return {
          coords: {
            x: gridItem.x * draw.canvasWidth,
            y: gridItem.y * draw.canvasHeight,
          },
          rgb: parseColor(color),
          ratio: Random.around(Math.PI / 4, 0.2),
          width: Random.between(5, 8) * draw.canvasMinSide * 0.08,
        };
      });
    });
  }
  ...
  draw = (draw: DrawContext) => {
    const imageData = draw.ctx.getImageData(
      0,
      0,
      draw.canvasWidth,
      draw.canvasHeight
    ...
    const canvasWidth = draw.canvasWidth;
